...
int main(int argc, char *argv[]) {
    ...
    do {
        printf("TCP Server Ready to Receive Inputs\n");
        client_length = sizeof(client_address);

        newSocket = accept(serverSocket, (struct sockaddr*) &client_address, &client_length);
        int line = read(newSocket, buffer, 129);
        buffer[line] = '\0';
        ...
        if (!isValidString(buffer)) {
            line = write(newSocket,"From server: Sorry, cannot compute!\n",37);
        }
        else {
            int x = sumUp(buffer);
            char response[150] = "";
            ...
            snprintf(response, sizeof(response), "From server: %d\n", x);
            strcat(final_string,response);
            line = write(newSocket,final_string,sizeof(final_string));
        }
